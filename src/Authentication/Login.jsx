      ...
      <div className="w-full max-w-6xl grid lg:grid-cols-2 gap-10 items-center">
        ...
        <div className="w-full">
          <div className="glass-effect rounded-xl shadow-lg p-6 lg:p-8 border border-white/60">
            <Link to="/" className="lg:hidden flex items-center justify-center mb-5 hover:opacity-80 transition-opacity">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleInputChange}
                    className={`w-full pl-9 pr-2.5 py-2 bg-white border rounded-lg focus:outline-none input-focus transition-all text-sm ${errors.email ? "border-red-400" : "border-slate-200"}`} placeholder="you@example.com" />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleInputChange}
                      className={`w-full pl-9 pr-9 py-2 bg-white border rounded-lg focus:outline-none input-focus transition-all text-sm ${errors.password ? "border-red-400" : "border-slate-200"}`} placeholder="••••••••" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-600">
                      {showPassword ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                  {errors.password && <p className="text-red-500 text-[10px] mt-0.5 flex items-center gap-1"><AlertCircle className="w-2.5 h-2.5" />{errors.password}</p>}
                </div>
              )}
              ...
              {!isForgotMode && (
                <div className="flex items-center justify-between pt-0.5">
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" className="w-3 h-3 rounded border-slate-300 text-slate-700 focus:ring-slate-400" />
                    <span className="text-[11px] text-slate-600 font-normal">Remember me</span>
                  </label>
                  <Link to="/forgot-password" className="text-[11px] font-medium text-slate-700 hover:text-slate-900 transition-colors">Forgot Password?</Link>
                </div>
              )}

              {isForgotMode && (
                <p className="text-[11px] text-slate-500 mt-1.5">We'll send you a link to reset your password</p>
              )}
              ...
              <button type="submit" disabled={isLoading}
                ...
                {isLoading ? (
                  <><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Processing...</>
                ) : (
                  <>{isForgotMode ? "Send Reset Link" : "Login"}<ArrowRight className="w-3.5 h-3.5" /></>
                )}
              </button>
            </form>
          ...
          <div className="text-center mt-5">
            <p className="text-slate-500 text-[11px] font-normal">&copy; {year} Inside Invoice by 2X+1. All rights reserved.</p>
